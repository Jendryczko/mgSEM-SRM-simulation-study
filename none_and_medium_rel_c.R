...
generalActorVar <- .1852942
generalPartnerVar <- .15
generalRelationshipVar <- .2647059
generalActorPartnerCov <- .03334314  #(implying correlation of .2)
generalRelationshipCov <- .02647059  #(implying correlation of .1)
...
differentActorVar <- .1852942
differentPartnerVar <- .15
differentRelationshipVar <- .2647059
differentActorPartnerCov <- .03334314  #(implying correlation of .2)
differentRelationshipCov <- .02647059  #(implying correlation of .1)
...
#1vs5 groups, groupsize 5, same, pv

propnmavector1vs5gs5same_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  bothgroupse <- mxModel("bothgroupse",
                         mxModel(base5, mxData(PopSame[random[1],], type="raw")), mxModel(other5same, mxData(PopSame[random[2:6],], type="raw")),
                         mxConstraint( (pv1 / (av1 + pv1 +rv1)) == (pv2 / (av2 + pv2 +rv2)) ),
                         mxFitFunctionMultigroup(c("base5", "other5same")))
  
  
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector1vs5gs5same_pv[i] <- -888
  }
    ...
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector1vs5gs5same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector1vs5gs5same_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector1vs5gs5same_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector1vs5gs5same_pv[i] <- -9999
  }
  else {
    propnmavector1vs5gs5same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector1vs5gs5same_pv, file = "propnmavector1vs5gs5same_pv.rda")
...
propnmavector1vs5gs5different_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector1vs5gs5different_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector1vs5gs5different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector1vs5gs5different_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector1vs5gs5different_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector1vs5gs5different_pv[i] <- -9999
  }
  else {
    propnmavector1vs5gs5different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector1vs5gs5different_pv, file = "propnmavector1vs5gs5different_pv.rda")
...
#1vs5 groups, groupsize 13, same, pv

propnmavector1vs5gs13same_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector1vs5gs13same_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector1vs5gs13same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector1vs5gs13same_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector1vs5gs13same_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector1vs5gs13same_pv[i] <- -9999
  }
  else {
    propnmavector1vs5gs13same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector1vs5gs13same_pv, file = "propnmavector1vs5gs13same_pv.rda")
...
propnmavector1vs5gs13different_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector1vs5gs13different_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector1vs5gs13different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector1vs5gs13different_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector1vs5gs13different_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector1vs5gs13different_pv[i] <- -9999
  }
  else {
    propnmavector1vs5gs13different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector1vs5gs13different_pv, file = "propnmavector1vs5gs13different_pv.rda")
...
#5vs5 groups, groupsize 5, same, pv

propnmavector5vs5gs5same_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector5vs5gs5same_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector5vs5gs5same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector5vs5gs5same_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector5vs5gs5same_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector5vs5gs5same_pv[i] <- -9999
  }
  else {
    propnmavector5vs5gs5same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector5vs5gs5same_pv, file = "propnmavector5vs5gs5same_pv.rda")
...
propnmavector5vs5gs5different_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector5vs5gs5different_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector5vs5gs5different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector5vs5gs5different_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector5vs5gs5different_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector5vs5gs5different_pv[i] <- -9999
  }
  else {
    propnmavector5vs5gs5different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector5vs5gs5different_pv, file = "propnmavector5vs5gs5different_pv.rda")
...
#5vs5 groups, groupsize 13, same, pv

propnmavector5vs5gs13same_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector5vs5gs13same_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector5vs5gs13same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector5vs5gs13same_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector5vs5gs13same_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector5vs5gs13same_pv[i] <- -9999
  }
  else {
    propnmavector5vs5gs13same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector5vs5gs13same_pv, file = "propnmavector5vs5gs13same_pv.rda")
...
propnmavector5vs5gs13different_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector5vs5gs13different_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector5vs5gs13different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector5vs5gs13different_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector5vs5gs13different_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector5vs5gs13different_pv[i] <- -9999
  }
  else {
    propnmavector5vs5gs13different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector5vs5gs13different_pv, file = "propnmavector5vs5gs13different_pv.rda")
...
#10vs10 groups, groupsize 5, same, pv

propnmavector10vs10gs5same_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector10vs10gs5same_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector10vs10gs5same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector10vs10gs5same_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector10vs10gs5same_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector10vs10gs5same_pv[i] <- -9999
  }
  else {
    propnmavector10vs10gs5same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector10vs10gs5same_pv, file = "propnmavector10vs10gs5same_pv.rda")
...
propnmavector10vs10gs5different_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector10vs10gs5different_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector10vs10gs5different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector10vs10gs5different_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector10vs10gs5different_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector10vs10gs5different_pv[i] <- -9999
  }
  else {
    propnmavector10vs10gs5different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector10vs10gs5different_pv, file = "propnmavector10vs10gs5different_pv.rda")
...
#10vs10 groups, groupsize 13, same, pv

propnmavector10vs10gs13same_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector10vs10gs13same_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector10vs10gs13same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector10vs10gs13same_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector10vs10gs13same_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector10vs10gs13same_pv[i] <- -9999
  }
  else {
    propnmavector10vs10gs13same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector10vs10gs13same_pv, file = "propnmavector10vs10gs13same_pv.rda")
...
propnmavector10vs10gs13different_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector10vs10gs13different_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector10vs10gs13different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector10vs10gs13different_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector10vs10gs13different_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector10vs10gs13different_pv[i] <- -9999
  }
  else {
    propnmavector10vs10gs13different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector10vs10gs13different_pv, file = "propnmavector10vs10gs13different_pv.rda")


###########################################################################################################################################


#20vs20 groups, groupsize 5, same, pv

propnmavector20vs20gs5same_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector20vs20gs5same_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector20vs20gs5same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector20vs20gs5same_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector20vs20gs5same_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector20vs20gs5same_pv[i] <- -9999
  }
  else {
    propnmavector20vs20gs5same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector20vs20gs5same_pv, file = "propnmavector20vs20gs5same_pv.rda")
...
propnmavector20vs20gs5different_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector20vs20gs5different_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector20vs20gs5different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector20vs20gs5different_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector20vs20gs5different_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector20vs20gs5different_pv[i] <- -9999
  }
  else {
    propnmavector20vs20gs5different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector20vs20gs5different_pv, file = "propnmavector20vs20gs5different_pv.rda")

#########################################################################################################################



#20vs20 groups, groupsize 13, same, pv

propnmavector20vs20gs13same_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector20vs20gs13same_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector20vs20gs13same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector20vs20gs13same_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector20vs20gs13same_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector20vs20gs13same_pv[i] <- -9999
  }
  else {
    propnmavector20vs20gs13same_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector20vs20gs13same_pv, file = "propnmavector20vs20gs13same_pv.rda")
...
propnmavector20vs20gs13different_pv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector20vs20gs13different_pv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector20vs20gs13different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector20vs20gs13different_pv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector20vs20gs13different_pv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector20vs20gs13different_pv[i] <- -9999
  }
  else {
    propnmavector20vs20gs13different_pv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector20vs20gs13different_pv, file = "propnmavector20vs20gs13different_pv.rda")
