...
generalActorVar <- .23333333
generalPartnerVar <- .1
generalRelationshipVar <- .5
generalActorPartnerCov <- .0305505  #(implying correlation of .2)
generalRelationshipCov <- .05       #(implying correlation of .1)
...
differentActorVar <- .23333333
differentPartnerVar <- .1
differentRelationshipVar <- .5
differentActorPartnerCov <- .0305505  #(implying correlation of .2)
...
#1vs5 groups, groupsize 5, same, rv

propnmavector1vs5gs5same_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector1vs5gs5same_rv[i] <- -888
  }
    ...
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector1vs5gs5same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector1vs5gs5same_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector1vs5gs5same_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector1vs5gs5same_rv[i] <- -9999
  }
  else {
    propnmavector1vs5gs5same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector1vs5gs5same_rv, file = "propnmavector1vs5gs5same_rv.rda")
...
propnmavector1vs5gs5different_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector1vs5gs5different_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector1vs5gs5different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector1vs5gs5different_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector1vs5gs5different_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector1vs5gs5different_rv[i] <- -9999
  }
  else {
    propnmavector1vs5gs5different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector1vs5gs5different_rv, file = "propnmavector1vs5gs5different_rv.rda")
...
#1vs5 groups, groupsize 13, same, rv

propnmavector1vs5gs13same_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector1vs5gs13same_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector1vs5gs13same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector1vs5gs13same_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector1vs5gs13same_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector1vs5gs13same_rv[i] <- -9999
  }
  else {
    propnmavector1vs5gs13same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector1vs5gs13same_rv, file = "propnmavector1vs5gs13same_rv.rda")
...
propnmavector1vs5gs13different_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector1vs5gs13different_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector1vs5gs13different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector1vs5gs13different_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector1vs5gs13different_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector1vs5gs13different_rv[i] <- -9999
  }
  else {
    propnmavector1vs5gs13different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector1vs5gs13different_rv, file = "propnmavector1vs5gs13different_rv.rda")
...
#5vs5 groups, groupsize 5, same, rv

propnmavector5vs5gs5same_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector5vs5gs5same_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector5vs5gs5same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector5vs5gs5same_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector5vs5gs5same_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector5vs5gs5same_rv[i] <- -9999
  }
  else {
    propnmavector5vs5gs5same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector5vs5gs5same_rv, file = "propnmavector5vs5gs5same_rv.rda")
...
propnmavector5vs5gs5different_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector5vs5gs5different_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector5vs5gs5different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector5vs5gs5different_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector5vs5gs5different_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector5vs5gs5different_rv[i] <- -9999
  }
  else {
    propnmavector5vs5gs5different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector5vs5gs5different_rv, file = "propnmavector5vs5gs5different_rv.rda")


############################################################################################################################


#5vs5 groups, groupsize 13, same, rv

propnmavector5vs5gs13same_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector5vs5gs13same_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector5vs5gs13same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector5vs5gs13same_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector5vs5gs13same_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector5vs5gs13same_rv[i] <- -9999
  }
  else {
    propnmavector5vs5gs13same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector5vs5gs13same_rv, file = "propnmavector5vs5gs13same_rv.rda")
...
propnmavector5vs5gs13different_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector5vs5gs13different_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector5vs5gs13different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector5vs5gs13different_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector5vs5gs13different_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector5vs5gs13different_rv[i] <- -9999
  }
  else {
    propnmavector5vs5gs13different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector5vs5gs13different_rv, file = "propnmavector5vs5gs13different_rv.rda")


############################################################################################################################


#10vs10 groups, groupsize 5, same, rv

propnmavector10vs10gs5same_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector10vs10gs5same_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector10vs10gs5same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector10vs10gs5same_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector10vs10gs5same_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector10vs10gs5same_rv[i] <- -9999
  }
  else {
    propnmavector10vs10gs5same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector10vs10gs5same_rv, file = "propnmavector10vs10gs5same_rv.rda")
...
propnmavector10vs10gs5different_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector10vs10gs5different_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector10vs10gs5different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector10vs10gs5different_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector10vs10gs5different_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector10vs10gs5different_rv[i] <- -9999
  }
  else {
    propnmavector10vs10gs5different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector10vs10gs5different_rv, file = "propnmavector10vs10gs5different_rv.rda")


############################################################################################################################


#10vs10 groups, groupsize 13, same, rv

propnmavector10vs10gs13same_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector10vs10gs13same_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector10vs10gs13same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector10vs10gs13same_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector10vs10gs13same_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector10vs10gs13same_rv[i] <- -9999
  }
  else {
    propnmavector10vs10gs13same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector10vs10gs13same_rv, file = "propnmavector10vs10gs13same_rv.rda")
...
propnmavector10vs10gs13different_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector10vs10gs13different_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector10vs10gs13different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector10vs10gs13different_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector10vs10gs13different_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector10vs10gs13different_rv[i] <- -9999
  }
  else {
    propnmavector10vs10gs13different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector10vs10gs13different_rv, file = "propnmavector10vs10gs13different_rv.rda")


############################################################################################################################


#20vs20 groups, groupsize 5, same, rv

propnmavector20vs20gs5same_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector20vs20gs5same_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector20vs20gs5same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector20vs20gs5same_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector20vs20gs5same_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector20vs20gs5same_rv[i] <- -9999
  }
  else {
    propnmavector20vs20gs5same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector20vs20gs5same_rv, file = "propnmavector20vs20gs5same_rv.rda")
...
propnmavector20vs20gs5different_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector20vs20gs5different_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector20vs20gs5different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector20vs20gs5different_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector20vs20gs5different_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector20vs20gs5different_rv[i] <- -9999
  }
  else {
    propnmavector20vs20gs5different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector20vs20gs5different_rv, file = "propnmavector20vs20gs5different_rv.rda")


############################################################################################################################


#20vs20 groups, groupsize 13, same, rv

propnmavector20vs20gs13same_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector20vs20gs13same_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector20vs20gs13same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector20vs20gs13same_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector20vs20gs13same_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector20vs20gs13same_rv[i] <- -9999
  }
  else {
    propnmavector20vs20gs13same_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector20vs20gs13same_rv, file = "propnmavector20vs20gs13same_rv.rda")
...
propnmavector20vs20gs13different_rv <- rep(NA, 1300)
...
for (i in 1:1300){
  ...
  if (has_warning(bothgroupsfit <- mxRunDave(bothgroups)) == T){
    propnmavector20vs20gs13different_rv[i] <- -888
  }
  else if (has_warning(bothgroupsefit <- mxRunDave(bothgroupse)) == T){
    if (grepl("optimality conditions to the required accuracy", summary(bothgroupsefit)[39]$npsolMessage) ==T){
      propnmavector20vs20gs13different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
    }
    else {
      propnmavector20vs20gs13different_rv[i] <- -8888}
  }
  else if (length(is(bothgroupsfit)) >  2){
    propnmavector20vs20gs13different_rv[i] <- -999
  }
  else if (length(is(bothgroupsefit)) >  2){
    propnmavector20vs20gs13different_rv[i] <- -9999
  }
  else {
    propnmavector20vs20gs13different_rv[i] <- summary(bothgroupsefit)[8]$Minus2LogLikelihood - summary(bothgroupsfit)[8]$Minus2LogLikelihood
  }
}
save(propnmavector20vs20gs13different_rv, file = "propnmavector20vs20gs13different_rv.rda")
